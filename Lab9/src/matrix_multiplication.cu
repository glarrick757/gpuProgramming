...
int main(int argc, char *argv[])
{
   ...
   //
   float *h_a, *h_c1, *h_c2, *h_b;  // In this application, we test matrix multiplication h_a * h_a and store results into h_c
   ...
   h_c1 = (float *)calloc(n * n, sizeof(float));
   h_c2 = (float *)calloc(n * n, sizeof(float));
  ...
  float *d_a = 0, *d_b = 0, *d_c1 = 0, *d_c2 = 0;
  cudaMalloc((void**)&d_a, sizeof(float) * n * n);
  cudaMalloc((void**)&d_b, sizeof(float) * n * n);
  cudaMalloc((void**)&d_c1, sizeof(float) * n * n);
  ...
  matrix_multiply_simple<<<num_blocks,block_size>>>(d_a, d_b, d_c1, n);
  cudaMemcpy(h_c1, d_c1, sizeof(float) * n * n, cudaMemcpyDeviceToHost);

  writeArray(h_c1, n, n, "gpuout2");
  if(shouldPrint)
      printArray(h_c1, n, n); 
  ...
  // to get accurate timings, launch a single "warm-up" kernel
  int shared_size = 2 * tile_width * tile_width * sizeof(float);
  printf("shared size %d\n", shared_size);
  matrix_multiply_improved<<<num_blocks,block_size,shared_size>>>(d_a, d_b, d_c2, n);
  cudaMemcpy(h_c2, d_c2, sizeof(float) * n * n, cudaMemcpyDeviceToHost);

  writeArray(h_c2, n, n, "gpuout3");
  if(shouldPrint)
      printArray(h_c2, n, n); 
  ...
  float speedup = average_simple_time / average_improved_time;
